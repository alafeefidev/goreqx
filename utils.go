...
// From https://cdn.jsdelivr.net/gh/microlinkhq/top-user-agents@master/src/desktop.json
var userAgents = []string{
	...
	"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Geg dko) obsidian/1.8.7 Chrome/130.0.6723.191 Electron/33.3.2 Safari/537.36",
...
func mapSlice(h map[string]string) [][2]string {
...
func cookieSlice(co []*http.Cookie) [][2]string {
	cookies := make([][2]string, 0, len(co))
	for _, c := range co {
		cookies = append(cookies, [2]string{c.Name, c.Value})
	}
	sort.Slice(cookies, func(i int, j int) bool {
		return cookies[i][0] < cookies[j][0]
	})
	return cookies
}

func formatJSON(j map[string]any) []byte {
	data, _ := json.Marshal(j)

	var strc any
	json.Unmarshal(data, &strc)
	result, _ := json.Marshal(strc)
	return result
}